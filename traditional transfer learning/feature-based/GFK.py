...
class GFK:
    ...
    def fit(self, Xs, Xt, norm_inputs=None):
        '''
        Obtain the kernel G
        :param Xs: num_source_sample * n_feature, source feature
        :param Xt: num_target_sample * n_feature, target feature
        :param norm_inputs: normalize the inputs or not
        :return: GFK kernel G
        '''
        if norm_inputs:
            source, mu_source, std_source = self.znorm(Xs)#标准化数据，平均值，标准差
        ...
        Ps = np.hstack((Ps.weights, scipy.linalg.null_space(Ps.weights.T)))#800*800
        Pt = Pt.weights[:, :self.dim]#n*20
        N = Ps.shape[1]
        dim = Pt.shape[1]
    ...
    def fit_predict(self, Xs, Xt, Ys, Yt):
        ...
        :param Xs: num_source_sample * n_feature, source feature
        :param Ys: num_source_sample * 1, source label
        :param Xt: num_target_sample * n_feature, target feature
        :param Yt: num_target_sample * 1, target label
